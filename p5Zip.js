function XMLpromise(url, type = "blob") {
	return new Promise(function(resolve, reject) {
		const request = new XMLHttpRequest();
		request.responseType = type;
		request.open("GET", url);
		request.onload = function() {
			if (this.status >= 200 && this.status < 300) { // good response
				resolve(request.response);
			} else {
				reject(`XML error; status: ${this.status}; error: ${this.statusText}`);
			}
		};
...
class p5Zip {
	constructor(url) {
		const pre = this.inPreload;
		this.zip = new JSZip.external.Promise((resolve, reject) => {
			XMLpromise(url + ".zip").then((data) => {
				return JSZip.loadAsync(data);
			}, err => reject(err)).then((data) => {
				if(pre) p5.instance._decrementPreload();

				resolve(data);
			}, err => reject(err))
		});

		if(pre) p5.instance._incrementPreload();
	...
	getImage(name, type = "png", onLoad = () => {}, onError = () => {}) {
		const pre = this.inPreload;
		const createURL = (window.URL || window.webkitURL).createObjectURL;
		...
		img.onload = () => {
			...
			p5Img.canvas.width = img.width;
			p5Img.canvas.height = img.height;
		...
		this.getFile(name, type, "blob").then((blob) => {
	...
	getModel(name, type = "obj", onLoad = () => {}, onError = () => {}) {
		const pre = this.inPreload;
		const model = new p5.Geometry();
		model.gid = `${name}.${type}|false`;

		this.getFile(name, type, "uint8array").then((content) => {
			if (type === "obj") {
				parseObj(model, this.decoder.decode(content).split("\n"));
				onLoad(model);
			} else if (type === "stl") {
				parseSTL(model, content.buffer);
				onLoad(model);
			} else {
				onError("Unsupported model file type");
			}
			if(pre) p5.instance._decrementPreload();
		}, onError);
	...
	getJSONarray(name, type = "json", onLoad = () => {}, onError = () => {}, expectObject = false) {
		return this.getJSON(name, type, onLoad, onError, !expectObject);
	}

	getJSON(name, type = "json", onLoad = () => {}, onError = () => {}, expectArray = false) {
		const pre = this.inPreload;
		const returnValue = expectArray ? [] : {};
		this.getFile(name, type, "string").then((content) => {
			const json = JSON.parse(content);
			for (const key in json) {
				returnValue[key] = json[key];
			}
	...
	getFile(name, type, format = "uint8array") {
		return new Promise((resolve, reject) => {
			this.zip.then((zip) => { // check the zip has been loaded
				const folder = this.subFolder != null ? this.subFolder + "/" : "";
				try {
					zip.file(folder + name + "." + type).async(format).then(
						resolve,
						reject)
				} catch (e) {
					reject("File " + folder + name + "." + type + " does not exist");
				}
			}, reject);
		});
	}
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
// ------------------------------------------------- STUPID FUDGING p5 closure made me copy this code -------------------------------------------------
// ----------------------------------------------------------------------------------------------------------------------------------------------------
